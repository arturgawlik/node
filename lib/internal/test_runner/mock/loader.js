  ...
  data?.port.on('message', ({ type, payload }) => {
    ...
    if (type === 'node:test:register') {
      ...
      mocks.set(baseURL, {
        ...
        preservedExportNames: payload.preservedExportNames,
...
async function createSourceFromMock(mock, format) {
  ...
  const { exportNames, hasDefaultExport, url, preserveOthers, preservedExportNames } = mock;
...
${preservedExportsSource(useESM, preserveOthers, preservedExportNames)}
...
function preservedExportsSource(useESM, preserveOthers, preservedExportNames) {
  let source = '';

  if (preserveOthers) {
    const hasDefault = ArrayPrototypeIncludes(preservedExportNames, "default");

    if (hasDefault) {
      if (useESM) {
        source += 'export default $__exports.mockedModuleOriginal["default"];\n';
      } else {
        source += 'module.exports = $__exports.mockedModuleOriginal["default"];\n';
      }
    }

    for (let i = 0; i < preservedExportNames.length; ++i) {
      const name = preservedExportNames[i];
      if (name === 'default') {
        continue;
      }

      if (useESM) {
        source += `export let ${name} = $__exports.mockedModuleOriginal[${JSONStringify(name)}];\n`;
      } else {
