...
// TODO AG: fix that
// I guess this should be symbol because we do not want to be used in user land??
const currentModuleParentSymbol = 'file://todo/this/module.js';
...
async function resolve(specifier, context, nextResolve) {
  ...
  if (mock.preserveOthers) {
    ...
    if (parentURL) {
      ...
      if (specifier === parentURL.href || currentModuleParentSymbol === parentURL.href) {
  ...
  // in fact we do not want to set additional flags, but just load original module,
  // so to do that we need to not set any flags
  ...
  // if (!mock.cache && !preservedExportLoad) { // `&& !preservedExportLoad` because there is no need to update localVersion when `preservedExportLoad` is `true` - because for such case we do not add this quey params (original is loaded)
  //   // With ESM, we can't remove modules from the cache. Bump the module's
  //   // version instead so that the next import will be uncached.
  //   mock.localVersion++;
  // }
...
async function load(url, context, nextLoad) {
  ...
  let hasMockSearchParam = false;
  if (parsedURL) {
    hasMockSearchParam = parsedURL.searchParams.has(kMockSearchParam);
  ...
  const returnOriginal = mock?.active !== true || !hasMockSearchParam;
  if (returnOriginal) {
    return original;
  }
  ...
  /**
   * We want to load it also for CJS, because CJS has a static analysis
   * that make it convenient to use imported CJS modules (https://nodejs.org/api/esm.html#commonjs-namespaces)
   * and those static analysis works only when exported symbols are explicity set on `module.exports`.
   */
  const originalModuleNamespace = await loadOriginalModuleNamespace(baseURL);
...
function preservedExportsSource(
  ...
  if (preserveOthers) {
    const originalModuleUrlWithParam = originalModuleUrl;
    ...
    if (useESM) {
    // TODO AG: is it correct? In `getReexportsFromOriginal` there are filter out `default` exports.
      ...
      source += `const $__originalFromPreservedOthers = require('${url}');\n`;
      for (const reexport of namedReexportsFromOriginal) {
        source += `module.exports.${reexport} = $__originalFromPreservedOthers.${reexport};\n`;
      }
    }
  }
...
async function loadOriginalModuleNamespace(originalModuleUrl) {
  ...
  const originalModuleNamespace = await getLoader().import(
    originalModuleUrl,
    currentModuleParentSymbol, // current file is set as parentUrl because we want to then just load original module instead of mocked one
    kEmptyObject,
  );
...
function getReexportsFromOriginal(originalModuleNamespace, exportNames) {
  const reexports = ObjectKeys(originalModuleNamespace).filter(
    (exportName) =>
      exportName !== 'default' && exportName !== 'module.exports' &&
      !ArrayPrototypeIncludes(exportNames, exportName),
  );
  if ('default' in originalModuleNamespace && typeof originalModuleNamespace.default === 'object') {
    // add here also `default` exports because they are needed when building
    // statically analyzable CJS exports
    for (const defaultExportKey of ObjectKeys(originalModuleNamespace.default)) {
      ArrayPrototypePush(reexports, defaultExportKey);
    }
  }
  return reexports;
}
