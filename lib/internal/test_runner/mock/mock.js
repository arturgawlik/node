...
function cjsMockModuleLoad(request, parent, isMain) {
  ...
  if (hasDefaultExport) {
    ...
    ObjectAssign(modExports, originalModuleNamespace);
  ...
  if (preserveOthers && !originalModuleIsDefault) {
    const originalModuleNames = ObjectKeys(originalModuleNamespace);
    for (let i = 0; i < originalModuleNames.length; ++i) {
      const name = originalModuleNames[i];
