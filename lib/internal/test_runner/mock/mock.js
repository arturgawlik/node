...
class MockTracker {
  ...
  module(specifier, options = kEmptyObject) {
    ...
    typeof preserveOthers !== 'undefined' && validateBoolean(preserveOthers, 'options.preserveOthers');
  ...
  if (preserveOthers && !originalModuleIsDefault) {
    ...
    for (let i = 0; i < namedReexportsFromOriginal.length; ++i) {
      ...
      if (!name in modExports) {
        const descriptor = ObjectGetOwnPropertyDescriptor(originalModuleNamespace, name);
        ObjectDefineProperty(modExports, name, descriptor);
      }
    }
  }
...
function getReexportsFromOriginal(originalModuleNamespace, exportNames) {
  return ObjectKeys(originalModuleNamespace).filter(exportName => exportName !== 'default' && !ArrayPrototypeIncludes(exportNames, exportName));
}
