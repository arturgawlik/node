...
class MockModuleContext {
  ...
  constructor({
    ...
    sharedState,
    preserveOthers,
    preservedExportNames,
    mockedModuleOriginal
  }) {
...
class MockTracker {
  ...
  async module(specifier, options = kEmptyObject) {
    ...
    const preservedExportNames = []
    let mockedModuleOriginal = null;
    if (preserveOthers) {
      mockedModuleOriginal = await sharedState.moduleLoader.import(specifier, caller, kEmptyObject);
      for (const key of ObjectKeys(mockedModuleOriginal)) {
        if (key in namedExports || (key === 'default' && hasDefaultExport)) {
          continue;
        }
        ArrayPrototypePush(preservedExportNames, key)
      }
    }
