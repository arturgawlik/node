...
const {
  codes: { ERR_INVALID_ARG_TYPE, ERR_INVALID_ARG_VALUE, ERR_INVALID_STATE },
...
const kMockPreserveOriginalSearchParam = 'node-test-preserve-original';
const kMockSuccess = 1;
const kMockExists = 2;
const kMockUnknownMessage = 3;
// const kWaitTimeout = 5_000;
const kWaitTimeout = 5000_000;
...
class MockTracker {
  ...
  module(specifier, options = kEmptyObject) {
    ...
    validateObject(options, 'options');
    debug(
      'module mock entry, specifier = "%s", options = %o',
      specifier,
      options
    );
    ...
    typeof preserveOthers !== 'undefined' &&
      validateBoolean(preserveOthers, 'options.preserveOthers');

    const sharedState = setupSharedModuleState();
    const mockSpecifier = StringPrototypeStartsWith(specifier, 'node:')
      ? StringPrototypeSlice(specifier, 5)
      : specifier;
...
function cjsMockModuleLoad(request, parent, isMain) {
  ...
  let originalModuleIsDefault = false;

  if (hasDefaultExport) {
    modExports = defaultExport;
  } else if (preserveOthers) {
    originalModuleNamespace = _load(request, parent, isMain);
    if (typeof modExports !== 'object' || modExports === null) {
      throw new Error(kBadExportsMessage);
    }

    // AG: needs to create new object because original is sealed
    // so it can't be extended in line 765
    modExports = { ...originalModuleNamespace };
    originalModuleIsDefault = true;
  }
  ...
  if (preserveOthers && !originalModuleIsDefault) {
    if (!originalModuleNamespace) {
      originalModuleNamespace = _load(request, parent, isMain);
      if (typeof modExports !== 'object' || modExports === null) {
        throw new Error(kBadExportsMessage);
      }
    ...
    for (let i = 0; i < namedReexportsFromOriginal.length; ++i) {
      const name = namedReexportsFromOriginal[i];

      if (!name in modExports) {
...
function getReexportsFromOriginal(originalModuleNamespace, exportNames) {
  return ObjectKeys(originalModuleNamespace).filter(
    (exportName) =>
      exportName !== 'default' &&
